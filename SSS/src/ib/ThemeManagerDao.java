...
public class ThemeManagerDao {
	
	private static ThemeManagerDao instance = new ThemeManagerDao();
	
	public static ThemeManagerDao getInstance() {
			return instance;
	}
	
	private ThemeManagerDao() {}
		...
		try {
			...
			if(theme.getParentId() == 0) {
				...
				rsGroup = stmtGroup.executeQuery(
						"select max(GROUP_ID) from THEME_MESSAGEjj");
				int maxGroupId = 0;
				if(rsGroup.next()) {
				...
				pstmtOrder = conn.prepareStatement(
						"select max(ORDER_NO) from THEME_MESSAGEjj "+
						"where PARENT_ID = ? THEME_MESSAGE_ID = ?");
			...
			if(theme.getOrderNo() > 0) {
				pstmtOrderUpdate = conn.prepareStatement(
						"update THEME_MESSAGEjj set ORDER_NO = ORDER_NO + 1 "+
			...
			// 새로운 글의 번호를 구한다.
			theme.setId(Sequencer.nextId(conn, "THEME_MESSAGEjj"));
			//글 삽입
			pstmtInsertMessage = conn.prepareStatement(
					"insert into THEME_MESSAGEjj values (?,?,?,?,?,?,?,?,?,?,?)");
			...
			pstmtInsertContent = conn.prepareStatement(
					"insert into THEME_CONTENTjj values (?,?)");
	...
	public void update(Theme theme) throws Exception{
		...
		try {
			...
			pstmtUpdateMessage = conn.prepareStatement("update THEME_MESSAGEjj set NAME=?, EMAIL=?, IMAGE=?, TITLE=? "
					+ "where THEME_MESSAGE_ID=?");
			pstmtUpdateContent = conn.prepareStatement("update THEME_CONTENTjj set CONTENT=? "
					+ "where THEME_MESSAGE_ID=?");
	...
	public int count(List whereCond, Map valueMap) throws Exception{
	      ...
	      try {
	         ...
	         query.append("select count(*) from THEME_MESSAGEJJ ");
	...
	public List selectList(List whereCond, Map valueMap, int startRow, int endRow) throws Exception{
		...
		try {
			...
			query.append("		from THEME_MESSAGEjj ");
			if(whereCond != null && whereCond.size() > 0) {
				query.append(" where ");
				for(int i = 0; i < whereCond.size(); i++) {
					query.append(whereCond.get(i));
	 ...
	 public Theme select(int id) throws Exception{
		   ...
		   ResultSet rsContent = null;
		      ...
		      try {
		         ...
		         conn = getConnection();
		         pstmtMessage = conn.prepareStatement("select * from THEME_MESSAGEJJ " + "where THEME_MESSAGE_ID = ?");
		     ...
		     if(rsMessage.next()) {
		        ...
		        pstmtContent = conn.prepareStatement("select CONTENT from THEME_CONTENTJJ " + "where THEME_MESSAGE_ID = ?");
		      ...
		      public void delete(int id) throws Exception{
		          ...
		          try {
		             ...
		             pstmtMessage = conn.prepareStatement("delete from THEME_MESSAGEJJ where THEME_MESSAGE_ID = ?");
		             pstmtContent = conn.prepareStatement("delete from THEME_CONTENTJJ where THEME_MESSAGE_ID = ?");
